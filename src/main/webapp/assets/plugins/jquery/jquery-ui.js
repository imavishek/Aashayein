...
(function() {
	...
	$.widget("ui.tabs", $.ui.tabs,
	{
		/*  Reload active tab (we assume it was remote-loaded)  */
		reloadActive: function() {
			this.load( this.option("active") );
		},

		/*
			Set the tab anchor label from it's data-title-template attribute. For example if we have the following
			<ul>
				...
				<li><a href="dummy.cfm" data-title-template="Number of files {0}">Number of files 1</a></li>
			we can call setTitleFromTemplate(li, "2") to have the label change to "Number of files 2"
		*/
		setTitleFromTemplate: function(tabLI, title) {
			var a = $("a", tabLI),
				templ = a.data("titleTemplate");

			if( typeof templ === 'undefined' ) {
				console.warn("Tab anchor doesn't have a data-title-template attribute.")
				return;
			}

			a.html( formatEx(templ, title) );
			a.effect("highlight", {color: '##ffff00'}, 1000);
		},

		setActiveTitleFromTemplate: function(title) {
			this.setTitleFromTemplate(this.active, title)
		},

		setActiveTitle: function(title) {
			$("a", this.active).html( title );
		}
	});
})();/**
